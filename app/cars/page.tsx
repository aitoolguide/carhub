...
// //       {cars.map((car) => (
// //         <Card key={car.id} className="cursor-pointer" onClick={() => onSelectCar(car)}>
// //           <img src={car.imageUrl} alt={car.title} className="w-full h-48 object-cover" />
// //           <div className="p-4">
...
const Card = ({ children, className, onClick }) => (
  <div onClick={onClick} className={cn('bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden cursor-pointer', className)}>
    {children}
  </div>
);
...
const Button = ({ children, className, onClick, disabled = false, variant = 'primary', ...props }) => {
...
const Select = ({ children, className = '', ...props }) => {
  const baseStyles = 'block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md';
  return (
...
const FilterSection = ({ title, children, open, onToggle }) => (
  <div className="border-b border-gray-200">
    <button onClick={onToggle} className="w-full flex justify-between items-center py-4 px-4 text-slate-700 hover:bg-slate-50 transition-colors">
...
const CheckboxFilterGroup = ({ title, items, selectedItems, onToggle }) => (
...
const getAvailableYears = () => {
    const years = mockCars.filter(car => car.type === 'car').map(car => car.year);
...
const CarCardGrid = ({ car, onClick }) => (
  <Card onClick={() => onClick(car)} className="flex flex-col h-full">
    <div className="relative">
      <img src={car.imageUrl} alt={`${car.year} ${car.make} ${car.model}`} className="w-full h-48 object-cover rounded-t-xl" />
      <button className="absolute top-2 right-2 bg-white rounded-full p-2 text-slate-500 hover:text-red-500 transition-colors">
        <Heart size={16} />
      </button>
    </div>
    <div className="p-4 flex flex-col justify-between flex-1">
      <div>
        ...
        <p className="text-xl font-bold text-slate-700 mt-2">${car.price.toLocaleString('en-US')}</p>
      ...
      <div className="mt-4 flex flex-wrap gap-2 items-center">
        ...
        <Badge className="bg-slate-100 text-slate-700">{car.dealer}</Badge>
      </div>
    </div>
  </Card>
);
...
const CarCardList = ({ car, onClick }) => (
  <Card onClick={() => {onClick(car)}} className="flex flex-col sm:flex-row overflow-hidden shadow-lg hover:shadow-xl transition-shadow">
    <div className="w-full sm:w-1/3 flex-shrink-0 relative">
      <img src={car.imageUrl} alt={`${car.year} ${car.make} ${car.model}`} className="w-full h-48 sm:h-full object-cover" />
      <button className="absolute top-2 left-2 bg-white rounded-full p-2 text-slate-500 hover:text-red-500 transition-colors">
        <Heart size={16} />
      </button>
    </div>
    <div className="w-full sm:w-2/3 p-4 flex flex-col justify-between">
      <div>
        <h3 className="text-lg font-bold text-slate-900">{car.year} {car.make} {car.model}</h3>
        <p className="text-2xl font-bold text-slate-700 mt-2">${car.price.toLocaleString('en-US')}</p>
        <div className="flex items-center mt-2 text-sm text-slate-500">
          <Gauge size={14} className="mr-1" />
          <span>{car.mileage.toLocaleString('en-US')} km</span>
        </div>
...
const CarDetailPage = ({ car, onBackClick }) => (
...
export default function App() {
 ...
 const router = useRouter();
  ...
  const filteredCars = mockCars.filter(car => {
    ...
    // Condition filter
    if (activeFilters.condition.length > 0 && !activeFilters.condition.includes(car.condition)) return false;
    ...
    // Body Type filter
    if (activeFilters.bodyType.length > 0 && !activeFilters.bodyType.includes(car.bodyType)) return false;

    // Mileage filter
    if (car.mileage < activeFilters.minMileage || car.mileage > activeFilters.maxMileage) return false;

    // Drivetrain filter
    if (activeFilters.drivetrain.length > 0 && !activeFilters.drivetrain.includes(car.drivetrain)) return false;

    // Fuel Type filter
    if (activeFilters.fuelType.length > 0 && !activeFilters.fuelType.includes(car.fuelType)) return false;
    ...
    // Features filter
    if (activeFilters.features.length > 0 && !activeFilters.features.every(feature => car.features.includes(feature))) return false;
      ...
      <div className="flex flex-col md:flex-row min-h-screen">
        ...
        <div className="w-full md:w-80 bg-white border-r border-gray-200 flex-shrink-0">
          <div className="p-4 border-b border-gray-200 flex justify-between items-center">
            ...
            <Button variant="link" onClick={clearFilters} className="text-xs">Clear</Button>
          </div>
        ...
        <div className="flex-1 p-4 md:p-8 overflow-y-auto">
          ...
          <div className={cn("gap-6", view === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" : "flex flex-col")}>
            {paginatedCars.map((car) =>
              car.type === 'car' ? (view === 'grid' ? <CarCardGrid key={car.id} car={car} onClick={()=>{router.push(`/cars/${car.id}`)}} /> : <CarCardList key={car.id} car={car} onClick={()=>{router.push(`/cars/${car.id}`)}} />) : <PromoCard key={car.id} />
            )}
          </div>
